...
# Το bucketing του codec name — ίδιο με το bucketCodec() στο attachmentC.ts και με
# το A-LEVEL "CallCodecTypeUsageGSM.sql". Ζει εδώ σαν σταθερά γιατί το /api/calls
# το χρειάζεται δύο φορές (SELECT + GROUP BY) στο ίδιο statement.
CODEC_BUCKET = """CASE
...
def list_calls(
    ...
    """Λίστα κλήσεων του "All Sessions" tab.

    ΔΟΜΗ (γιατί δεν είναι ένα ενιαίο SELECT): τα per-session μεγέθη (MOS, setup-time
    KPIs, codec buckets, BadCall, low-MOS) υπολογίζονταν με correlated OUTER APPLY,
    δηλαδή μία εκτέλεση ανά κλήση — σε 3.5k κλήσεις αυτό έκανε ~11s SQL (μόνο το
    vResultsKPI OUTER APPLY ~5.7s, το codec ~6s). Τώρα κάθε μέγεθος υπολογίζεται
    ΜΙΑ φορά, set-based, σε #temp table με clustered index στο SessionId, και το
    τελικό SELECT απλώς κάνει LEFT JOIN. Ίδια ακριβώς αποτελέσματα (επαληθεύτηκε
    column-by-column σε CRETE_26H2), ~4x πιο γρήγορα.

    Τα #temp tables ζουν στο session της σύνδεσης και πέφτουν με το conn.close().
    """
    try:
        ...
        # AnalysisComment.Comment / DwAnalysisCommentToSessionMapping.Comment είναι
        # varchar(MAX). Ένα LOB column στο result set βάζει τον ODBC driver σε
        # per-row SQLGetData: το ΙΔΙΟ τελικό SELECT κόστιζε ~2.3s με το raw COALESCE
        # και ~0.12s με CAST σε bounded varchar. 1000 chars είναι άφθονα για ένα
        # analysis comment (τα υπάρχοντα είναι <30 chars, το Sessions.InvalidReason
        # είναι ούτως ή άλλως varchar(255)).
        comment_expr = f"CAST({comment_expr} AS varchar(1000))"

        # ── φίλτρα: μπαίνουν ΜΟΝΟ στο #scope, όλα τα υπόλοιπα βήματα κρέμονται από αυτό ──
        params: list[object] = []
        scope_filters = ""
        ...
        if selected_collections:
            ...
            scope_filters += f" AND DF.CollectionName IN ({placeholders})"
        ...
        if selected_locations:
            ...
            scope_filters += f" AND DF.ASideLocation IN ({placeholders})"
            params.extend(selected_locations)

        # #scope: οι κλήσεις που ζητήθηκαν. Ένα row ανά SessionId (το SessionId είναι
        # μοναδικό στο CallAnalysis). Κρατάει FileId/StartTs για το #nettech.
        #
        # ΠΡΟΣΟΧΗ γιατί είναι σπασμένο σε δύο execute: όταν το pyodbc στέλνει statement
        # ΜΕ parameters, το τυλίγει σε sp_executesql — αυτό τρέχει σε δικό του scope και
        # το #temp table που θα δημιουργούσε εκεί πεθαίνει μόλις γυρίσει. Οπότε φτιάχνουμε
        # πρώτα τον άδειο πίνακα χωρίς parameters (WHERE 1 = 0, ώστε να πάρει τα σωστά
        # types) και μετά κάνουμε το INSERT με τα φίλτρα — το INSERT σε ήδη υπαρκτό
        # #temp δουλεύει κανονικά μέσα από sp_executesql.
        scope_select = """
            SELECT CA.SessionId, CA.FileId, COALESCE(S.startTime, SB.startTime) AS StartTs
        ...
        """
        cursor.execute(
            f"SET NOCOUNT ON; SELECT * INTO #scope FROM ({scope_select}) X WHERE 1 = 0;"
        )
        cursor.execute(
            f"""
            SET NOCOUNT ON;
            INSERT INTO #scope (SessionId, FileId, StartTs)
            {scope_select}
            WHERE (S.Valid = 1 or S.Valid = 0){scope_filters};
            """,
            tuple(params),
        )
        ...
        prep = f"""
            ...
            CREATE CLUSTERED INDEX IX_scope ON #scope(SessionId);

            -- Avg_mos: χοντρικός μέσος όρος OptionalWB της κλήσης, χωρίς κανένα φίλτρο
            -- (ό,τι έκανε και το παλιό scalar subquery).
            SELECT LQ.SessionId, ROUND(AVG(LQ.OptionalWB), 2) AS AvgMos
            INTO #avgmos
            FROM ResultsLQ08Avg LQ
            WHERE EXISTS (SELECT 1 FROM #scope s WHERE s.SessionId = LQ.SessionId)
            GROUP BY LQ.SessionId;
            CREATE CLUSTERED INDEX IX_avgmos ON #avgmos(SessionId);
            ...
            WHERE TI.Valid = 1 AND LQ.OptionalWB BETWEEN 1 AND 5
              AND EXISTS (SELECT 1 FROM #scope s WHERE s.SessionId = LQ.SessionId)
            GROUP BY LQ.SessionId;
            CREATE CLUSTERED INDEX IX_mos ON #mos(SessionId);

            -- Session's radio band (π.χ. "GSM 900", "UMTS 2100") για το MOC/MTC setup
            -- time φίλτρο παρακάτω — ίδιο "latest NetworkInfo πριν το session start"
            ...
            SELECT s.SessionId, ni.Technology
            INTO #nettech
            FROM #scope s
            OUTER APPLY (
                SELECT TOP (1) n.Technology
                FROM NetworkInfo AS n
                WHERE n.FileId = s.FileId
                  AND n.MsgTime < s.StartTs
            ...
            -- Το vVoiceCodecTest είναι view πάνω σε view με correlated MAX(Duration) και
            -- scalar UDF (dbo.GetCodec) στο CodecName. Το υλοποιούμε ΜΙΑ φορά για το scope
            -- σε #vc· αλλιώς ο optimizer ξανα-αποτιμά το CodecName για καθεμιά από τις 12
            -- αναφορές του codec bucket (2.3s -> 1.0s).
            SELECT v.TestId, v.Direction, CAST(v.CodecName AS varchar(64)) AS CodecName
            INTO #vc
            FROM vVoiceCodecTest v
            WHERE EXISTS (SELECT 1 FROM #scope s WHERE s.SessionId = v.SessionId);
            CREATE CLUSTERED INDEX IX_vc ON #vc(TestId, Direction);
            ...
            SELECT TI2.SessionID AS SessionId, {CODEC_BUCKET} AS CodecBucket, COUNT(*) AS Cnt
            INTO #codecbucket
            FROM TestInfo TI2
            JOIN ResultsLQ08Avg R2 ON TI2.TestId = R2.TestId AND R2.Appl % 10 <> 0
            LEFT JOIN #vc VVCT ON TI2.TestID = VVCT.TestID AND (
                (TI2.direction = 'A->B' AND VVCT.Direction = 'U') OR
                (TI2.direction = 'B->A' AND VVCT.Direction = 'D')
            )
            WHERE TI2.Valid = 1
              AND EXISTS (SELECT 1 FROM #scope s WHERE s.SessionId = TI2.SessionID)
            GROUP BY TI2.SessionID, {CODEC_BUCKET};

            SELECT SessionId,
                SUM(CASE WHEN CodecBucket = 'EVS' THEN Cnt ELSE 0 END)           AS CodecEvsCount,
                SUM(CASE WHEN CodecBucket = 'EVS WB' THEN Cnt ELSE 0 END)        AS CodecEvsWbCount,
                SUM(CASE WHEN CodecBucket = 'AMR UMTS' THEN Cnt ELSE 0 END)      AS CodecAmrUmtsCount,
                SUM(CASE WHEN CodecBucket = 'AMR FR' THEN Cnt ELSE 0 END)        AS CodecAmrFrCount,
                SUM(CASE WHEN CodecBucket = 'AMR WB' THEN Cnt ELSE 0 END)        AS CodecAmrWbCount,
                SUM(CASE WHEN CodecBucket = 'AMR HR' THEN Cnt ELSE 0 END)        AS CodecAmrHrCount,
                SUM(CASE WHEN CodecBucket = 'AMR' THEN Cnt ELSE 0 END)           AS CodecAmrCount,
                SUM(CASE WHEN CodecBucket = 'EFR' THEN Cnt ELSE 0 END)           AS CodecEfrCount,
                SUM(CASE WHEN CodecBucket = 'FR' THEN Cnt ELSE 0 END)            AS CodecFrCount,
                SUM(CASE WHEN CodecBucket = 'HR' THEN Cnt ELSE 0 END)            AS CodecHrCount,
                SUM(CASE WHEN CodecBucket = 'other' THEN Cnt ELSE 0 END)         AS CodecOtherCount,
                SUM(CASE WHEN CodecBucket = 'no codec rate' THEN Cnt ELSE 0 END) AS CodecNoRateCount
            INTO #codec
            FROM #codecbucket
            GROUP BY SessionId;
            CREATE CLUSTERED INDEX IX_codec ON #codec(SessionId);
            ...
            -- έγκυρα δείγματα ξεπερνά 15%. Ίδιο Valid/Appl φίλτρο με το #mos
            ...
            -- επαληθευτεί 1:1 σαν το MOC/MTC/VoLTE/CS setup time, βλ. #vkpi).
            SELECT SessionId,
                CASE
                    WHEN SUM(ValidSample) > 0
                    THEN CASE
                            WHEN CONVERT(REAL, SUM(BadSample)) * 100.0 / CONVERT(REAL, SUM(ValidSample)) > 15.0
                            THEN 1 ELSE 0
                         END
                    ELSE NULL
                END AS BadCall,
                CASE
                    WHEN SUM(ValidSample) > 0
                    THEN CONVERT(REAL, SUM(BadSample)) * 100.0 / CONVERT(REAL, SUM(ValidSample))
                    ELSE NULL
                END AS BadCallPct,
                SUM(BadSample)     AS NumBadSample,
                SUM(ValidSample)   AS NumValidSample,
                SUM(SilenceSample) AS NumSilenceSample
            INTO #badcall
            FROM (
                SELECT LQ.SessionId,
                    CASE WHEN LQ.OptionalWB < 2.2 OR SUBSTRING(REVERSE(LQ.QualityCode), 10, 1) LIKE '1' THEN 1 ELSE 0 END AS BadSample,
                    CASE WHEN LQ.OptionalWB > 0   OR SUBSTRING(REVERSE(LQ.QualityCode), 10, 1) LIKE '1' THEN 1 ELSE 0 END AS ValidSample,
                    CASE WHEN SUBSTRING(REVERSE(LQ.QualityCode), 10, 1) LIKE '1' THEN 1 ELSE 0 END AS SilenceSample
                FROM ResultsLQ08Avg LQ
                JOIN TestInfo TI ON TI.TestId = LQ.TestId
                WHERE TI.Valid = 1
                  AND LQ.Appl % 10 <> 0
            ...
            GROUP BY SessionId;
            CREATE CLUSTERED INDEX IX_badcall ON #badcall(SessionId);

            -- "Low Speech Quality Calls (POLQA < 1.3)" — ίδιο κριτήριο με το A-LEVEL
            -- "LOW MOS 1_3.sql" reference query: ένα session πιάνεται αν υπάρχουν 3
            -- διαδοχικά TestId δείγματα (L1, L1+1, L1+2) όπου το L1 είναι "κακό" ΚΑΙ
            -- (το L1+1 ή το L1+2 είναι επίσης "κακό") — "κακό" σημαίνει OptionalWB
            -- αυστηρά μέσα στο (1.01, 1.29) ή status='Silence' με ένα από τα δύο
            -- συγκεκριμένα QualityCode. Το "μόνο Completed κλήσεις" της reference
            -- μπαίνει στο τελικό SELECT (εξαρτάται από το CA.callStatus). Διαφορετικό
            -- silence-flag test από το BadCall παραπάνω (bit) — έτσι ακριβώς το ορίζει
            -- η reference query, δεν το ενοποιούμε.
            SELECT DISTINCT L1.SessionId
            INTO #badquality
            ...
            WHERE EXISTS (SELECT 1 FROM #scope s WHERE s.SessionId = L1.SessionId)
              AND (
            ...
            CREATE CLUSTERED INDEX IX_badquality ON #badquality(SessionId);
        """
        ...
        query = f"""
            SELECT
                ...
                -- και ΔΕΝ ταιριάζει 1:1 με το pivot — βλ. #vkpi). Callstatus in
                ...
                CASE WHEN CA.callStatus = 'Completed' AND BADQUALITY.SessionId IS NOT NULL
                ...
                S.Valid AS isValid
            FROM CallAnalysis CA
            INNER JOIN #scope SC ON SC.SessionId = CA.SessionId
            LEFT JOIN FileList DF ON CA.FileId = DF.FileId
            ...
            {dw_comment_join}
            LEFT JOIN #avgmos AVGMOS      ON AVGMOS.SessionId = CA.SessionId
            LEFT JOIN #mos MOS            ON MOS.SessionId = CA.SessionId
            LEFT JOIN #nettech NETTECH    ON NETTECH.SessionId = CA.SessionId
            LEFT JOIN #vkpi VKPI          ON VKPI.SessionId = CA.SessionId
...
def get_technology_mix(
    ...
    try:
        ...
        if selected_collections:
            placeholders = ", ".join(["?"] * len(selected_collections))
            file_filters += f" AND FL.CollectionName IN ({placeholders})"
            params.extend(selected_collections)

        selected_locations = [loc for loc in (location or []) if loc and loc.strip()]
        if selected_locations:
            placeholders = ", ".join(["?"] * len(selected_locations))
            file_filters += f" AND FL.ASideLocation IN ({placeholders})"
            params.extend(selected_locations)

        # Το "latest NetworkInfo πριν από κάθε Position" τρέχει μία φορά ΑΝΑ GPS θέση —
        # σε ένα campaign αυτό είναι >1M εκτελέσεις. Το NetworkInfo δεν έχει index στο
        # (FileId, MsgTime), οπότε κάθε μία από αυτές έκανε key lookup + sort: ~9s.
        # Αντιγράφουμε πρώτα τα 3 columns που χρειαζόμαστε σε #ni με ακριβώς αυτό το
        # clustered index (μερικές δεκάδες χιλιάδες γραμμές, ένα scan) και το APPLY
        # γίνεται καθαρό seek: ~1s, ίδια ακριβώς αποτελέσματα.
        # Δύο execute για το #files: με parameters το pyodbc τρέχει μέσα από sp_executesql,
        # που έχει δικό του scope και θα έριχνε το #temp table μόλις γυρίσει (βλ. ίδιο
        # σχόλιο στο /api/calls).
        cursor.execute(
            "SET NOCOUNT ON; SELECT FL.FileId, FL.ASideLocation INTO #files FROM FileList FL WHERE 1 = 0;"
        )
        cursor.execute(
            f"""
            SET NOCOUNT ON;
            INSERT INTO #files (FileId, ASideLocation)
            SELECT FL.FileId, FL.ASideLocation FROM FileList FL WHERE 1 = 1{file_filters};
            """,
            tuple(params),
        )
        cursor.execute("""
            ...
            CREATE CLUSTERED INDEX IX_files ON #files(FileId);

            SELECT n.FileId, n.MsgTime, CAST(n.Technology AS varchar(64)) AS Technology
            INTO #ni
            FROM NetworkInfo n
            WHERE EXISTS (SELECT 1 FROM #files f WHERE f.FileId = n.FileId);
            CREATE CLUSTERED INDEX IX_ni ON #ni(FileId, MsgTime);
        """)
        ...
        cursor.execute("""
            SELECT
                ...
                COUNT(*) AS samples
            FROM Sessions
            INNER JOIN #files F ON Sessions.FileId = F.FileId
            ...
            GROUP BY F.ASideLocation, ni.Technology
        """)
