...
FREE (LTE/NR) panels: RSRP, MOS, CALLS, TECH.
"""
...
from bi_quries import query_m_to_m


# ---------------------------------------------------------------------------
# Helper functions (duplicated across panel modules for simplicity)
# ---------------------------------------------------------------------------
...
def thin_equidistant(pts: List[Tuple[float, float]], max_pts: int = 120) -> List[Tuple[float, float]]:
    """Return at most max_pts points sampled (approximately) equidistantly along the list."""
    n = len(pts)
    if n <= max_pts:
        return pts
...
def add_values_groups(m: folium.Map, parent_name: str = "VALUES") -> Dict[str, folium.FeatureGroup]:
    ...
    groups = {
        ...
        "RSRP": FeatureGroupSubGroup(parent, "RSRP").add_to(m),
...
def _build_route_and_waypoints(
    ...
    g_route = FeatureGroupSubGroup(groups["ROUTE"], "ROUTE", show=True)
    m.add_child(g_route)
...
def make_panel_free(
    ...
    """
    Create one FREE map panel and return (map, counters).
    Counters:
      - RSRP_* buckets
      - LQ buckets
      - Call status buckets
    ...
    # recenter from bounds
    b = bounds(all_input_pts) if all_input_pts else None
    if b:
        m.location = [(b[0][0] + b[1][0]) / 2.0, (b[0][1] + b[1][1]) / 2.0]

    # TECHNOLOGY LAYER - with fallback
    actual_phone, df = _get_phone_fallback(phone, collection, database, query_technology_free)
    if actual_phone != phone:
        print(f"[TECHNOLOGY] Using fallback phone: {actual_phone}")
    ...
    if not df.empty:
        ...
        def tech_color(tech: str) -> str:
            ...
            if tech == "GSM 900":
                ...
                counters["GSM_1800"]+=1
                return "blue"
            elif tech == "LTE E-UTRA 1":
                counters["LTE_E-UTRA_1"]+=1
                return "brown"
            elif tech == "LTE E-UTRA 3":
                counters["LTE_E-UTRA_3"]+=1
                return "green"
            elif tech == "LTE E-UTRA 20":
                counters["LTE_E-UTRA_20"]+=1
                return "orange"
            elif tech == "LTE E-UTRA 28":
                counters["LTE_E-UTRA_28"]+=1
                return "purple"
            elif tech == "LTE E-UTRA 7":
                counters["LTE_E-UTRA_7"]+=1
                return "red"
            return "gray"

        # tech_counts = counters["TECH_COUNTS"]  # shorthand
        counter_rows = 0
        prev_tech=None
        for _, r in df.iterrows():
            tech_val = str(r.get("technology") or "Unknown").strip()
            # tech_counts[tech_val] = tech_counts.get(tech_val, 0) + 1  # <--- count
            counter_rows += 1
            color = tech_color(tech_val)
            if counter_rows % 4 == 0 or tech_val != prev_tech:
                folium.CircleMarker(
    ...
    # RSRP LAYER - with fallback
    actual_phone_rsrp, df_rsrp = _get_phone_fallback(phone, collection, database, query_rsrp_free)
    if actual_phone_rsrp != phone:
        print(f"[RSRP] Using fallback phone: {actual_phone_rsrp}")
    df_rsrp = df_rsrp.dropna(subset=["latitude", "longitude", "rsrp"])

    g_rsrp = FeatureGroupSubGroup(groups["RSRP"], "RSRP", show=False)
    m.add_child(g_rsrp)
    counter_rows = 0
    previous_color = None
    same_color_count = 0
    ...
    # CALLS + COUNTERS - with fallback
    actual_phone_calls, df_calls = _get_phone_fallback(phone, collection, database, query_calls_free)
    if actual_phone_calls != phone:
        print(f"[CALLS] Using fallback phone: {actual_phone_calls}")
    df_calls = df_calls[
    ...
    try:
        df_prev = query_m_to_m(phone, collection)
        ...
        if df_prev.empty and 'nova' in phone.lower():
            ...
            df_prev = query_m_to_m(fallback_phone, collection)
    ...
    # rx level for free - with fallback
    actual_phone_rx, df_rx = _get_phone_fallback(phone, collection, database, query_rx_lev_gsm)
    if actual_phone_rx != phone:
        print(f"[RX] Using fallback phone: {actual_phone_rx}")
    ...
    # MOS LAYER - with fallback
    actual_phone_mos, df_mos = _get_phone_fallback(phone, collection, database, query_mos_free)
    if actual_phone_mos != phone:
        print(f"[MOS] Using fallback phone: {actual_phone_mos}")
    df_mos = df_mos[
    ...
    # Μέσα στην make_panel_free, στο τέλος πριν το return:

# 1. Συλλογή σημείων από SQL DataFrames αν το GPX είναι άδειο
    sql_points = []
    if not df.empty:
        sql_points.extend(df[['latitude', 'longitude']].values.tolist())

    elif not df_rsrp.empty:
        sql_points.extend(df_rsrp[['latitude', 'longitude']].values.tolist())
