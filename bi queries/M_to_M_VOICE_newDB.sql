...
from 	#tmpsession s, CallAnalysis c, networkInfo, networkIdRelation, Position
where 	s.SessionId = c.SessionId AND
		c.NetworkId = networkInfo.networkId AND
		c.callType in ('M->M') AND
...
--------------------------------------------------------------------------------------------------------------
	...
	-- V
	case 
		when c.callDir='A->B' and c.callStatus in ('dropped','completed') then 
			case when #tmpCallSetupResults.Sum_MO_Duration_11000!='' then 
				convert(varchar, #tmpCallSetupResults.Sum_MO_Duration_11000/1000.0) 
			else '' end

		when c.callDir='B->A' and c.callStatus in ('dropped','completed') then 
			case when (
				select max(br.Sum_MO_Duration)
				from #tmpBResults br
				where br.sessionIdA = s.sessionId
				  and br.kpiid = 11000
			) is not null then 
				convert(varchar, #tmpBResults.Sum_MO_Duration /1000.0)
...
#tmpsession     Join sessions s On(#tmpsession.sessionID = s.sessionID)
				--join #tmpsessionB sb on(s.SessionId = sb.SessionId)  ---EDW 
	        Join callSession c On(s.sessionID = c.sessionID)	
...
--AND SessionId in ('1275605286918')
